...
The Ladder Diagram (LD) is the language used for creating ladder logic. The diagram looks like a two-rail ladder with horizontal "rungs" between the two vertical "rails". These "rails" bound the LD networks from the left and right. The left rail is always considered ON (powered), while the right rail doesn't have a defined state.
...
The elements are the components between the rails. They can be horizontal or vertical. Each of these elements has a state, either ON or OFF, corresponding to the literal Boolean values of 1 or 0 respectively.
