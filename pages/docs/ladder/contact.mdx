...
The state of the left link is copied to the right link if the state of the associated boolean variable is **TRUE**. Otherwise, the state of the right link is **FALSE**.
To put it another way, it acts like a switch that is normally open (off) and closes (on) when the associated condition is **TRUE**.
...
The state of the right link is **TRUE** from one evaluation of this element to the next when a transition of the associated boolean variable from
**FALSE** to **TRUE** is sensed at the same time that the state of the left link is **TRUE**. The state of the right link shall be **FALSE** at all other times.
...
The state of the right link is **TRUE** from one evaluation of this element to the next when a transition of the associated boolean variable from
**TRUE** to **FALSE** is sensed at the same time that the state of the left link is **TRUE**. The state of the right link shall be **FALSE** at all other times.
